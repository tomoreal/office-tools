...
def _read_numeric(ws, row, col):
    """
    セルの実数値を返す。
    SUM式セルの場合、Excelが評価前であっても独自に数式または範囲を解析して合算値を返す。
    """
    ...
    if isinstance(val, str) and val.startswith('=SUM('):
        import re
        # =SUM(A1:B1) または =SUM(A1,B1,C1) の形式を抽出
        content = val[5:-1]
        total = 0.0
        has_val = False
        
        # カンマで区切られたパーツ（範囲または単一セル）を処理
        parts = content.split(',')
        for part in parts:
            if ':' in part:
                # 範囲形式: A1:C1
                try:
                    ...
                    # 列文字のみを抽出して数値に変換 (行は row を固定で使用)
                    start_col = column_index_from_string(re.sub(r'\d+', '', start_cell))
                    end_col = column_index_from_string(re.sub(r'\d+', '', end_cell))
                    for c in range(start_col, end_col + 1):
                        v = _read_numeric(ws, row, c)
                        if v is not None:
                            total += v
                            has_val = True
                except Exception:
                ...
                # 単一セル形式: A1
                try:
                    c = column_index_from_string(re.sub(r'\d+', '', part))
                    # 自分自身を指している場合はスキップ（無限ループ防止）
                    if c == col:
                        continue
                    v = _read_numeric(ws, row, c)
                    if v is not None:
...
def _make_get_val_for_filing(analysis_ws, col_to_dim,
    ...
    def _get_val_for_filing(src_row, c, fp, is_current):
        ...
        # 合計列かつフィルタがない場合、または既に数値（XBRL等の上書き）がある場合は、
        # analysis_ws の値をそのまま使用する（再計算による2重計上を防ぐため）
        if c in (hokoku_col, goukei_col):
            # フィルタがない場合は、事前計算済みの値を優先
            if not allowed:
                v = _read_numeric(analysis_ws, src_row, c)
                if v is not None:
                    return v
...
def _create_data_acquisition_sheet(workbook, analysis_sheet_name, used_sheet_names,
    ...
    # --- 「報告セグメント」(XBRLオリジナル)を検出し、
    #     「報告セグメント以外の全てのセグメント」の直前に移動して名称を変更する ---
    orig_hokoku_pos = next((i for i, h in enumerate(seg_headers) if str(h) == "報告セグメント"), None)
    igai_zen_pos = next((i for i, h in enumerate(seg_headers) if "報告セグメント以外の全てのセグメント" in str(h)), None)

    if igai_zen_pos is not None and orig_hokoku_pos is not None:
...
def _create_ppm_analysis_sheet(workbook, analysis_sheet_name, used_sheet_names, filing_pairs, debug_log,
    ...
    # -----------------------------------------------------------------------
    # 3b. 列の再構成と合計列の挿入（1.報告 2.合計 3.その他 4.合計 5.調整 6.全社 7.既存合計）
    # -----------------------------------------------------------------------
    # キーワード定義
    OTHER_KW = ["その他", "以外", "Other"]
    ADJ_KW   = ["調整", "消去", "内部", "Adjustment", "Elimination"]
    TOTAL_KW = ["合計", "計", "Total", "全社", "全体", "連結", "単体"]

    def _get_category(col_idx):
        val = str(analysis_ws.cell(1, col_idx).value or "")
        if val == "報告セグメント" or val == "全体":
            return "existing_total"
        if any(kw in val for kw in TOTAL_KW):
            return "existing_total"
        if any(kw in val for kw in ADJ_KW):
            return "adj"
        if any(kw in val for kw in OTHER_KW):
            return "other"
        return "reporting"

    # 全列の情報を一旦収集し、カテゴリ分けする
    all_col_data = []
    total_rows = analysis_ws.max_row
    for _ci in range(3, max_col + 1):
        cat = _get_category(_ci)
        header = analysis_ws.cell(1, _ci).value
        data = []
        for _ri in range(2, total_rows + 1):
            data.append(analysis_ws.cell(_ri, _ci).value)
        all_col_data.append({"header": header, "cat": cat, "original_idx": _ci, "data": data})

    # 元の列（3列目以降）を一旦削除する（後ろから削除）
    for _ci in range(max_col, 2, -1):
        analysis_ws.delete_cols(_ci)

    next_col = 3

    # カテゴリごとに再集計・再挿入
    def _append_col(header, data=None, formula_type=None, formula_range=None):
        nonlocal max_col, next_col
        new_idx = next_col
        analysis_ws.cell(1, new_idx).value = header
        if data:
            for i, val in enumerate(data):
                analysis_ws.cell(i + 2, new_idx).value = val
        elif formula_type == "sum" and formula_range:
            _sum_start = get_column_letter(formula_range[0])
            _sum_end   = get_column_letter(formula_range[1])
            for _ri in range(2, total_rows + 1):
                analysis_ws.cell(_ri, new_idx).value = f"=SUM({_sum_start}{_ri}:{_sum_end}{_ri})"
        elif formula_type == "sum_parts" and formula_range:
            _parts = [get_column_letter(c) for c in formula_range]
            for _ri in range(2, total_rows + 1):
                formula = "=SUM(" + ",".join([f"{p}{_ri}" for p in _parts]) + ")"
                analysis_ws.cell(_ri, new_idx).value = formula
        max_col = new_idx
        next_col += 1
        return new_idx
    ...
    # -----------------------------------------------------------------------
    # 実際のヘッダー文字列をPython変数に保持（Python側検索用）
    # analysis_ws rebuilt row 1 から直接取得（実文字列が保証される）
    # -----------------------------------------------------------------------
    ppm_col_header_strs = {}  # analysis col c -> 実際のヘッダー文字列
    for _c in range(3, analysis_ws.max_column + 1):
        _hv = analysis_ws.cell(1, _c).value
        if _hv is not None:
            ppm_col_header_strs[_c] = str(_hv)

    # igai_col の補完: analysis_ws ヘッダーから「報告セグメント以外」「その他」を検索
    if igai_col is None:
        for _c, _h in sorted(ppm_col_header_strs.items()):
            if (('以外' in _h and '報告セグメント' in _h) or
                    ('その他' in _h and '及びその他の合計' not in _h)):
                if _c != hokoku_col and _c != goukei_col:
                    igai_col = _c
                    debug_log(f"[PPM Analysis] igai_col補完: col={igai_col}, header='{_h}'")
                    break

    debug_log(f"[PPM Analysis] acq_ws_ref={'exists' if acq_ws_ref else 'None'}, "
              f"acq_sales_label='{_acq_sales_label}', "
              f"ppm_loop_end={_ppm_loop_end}, igai_col={igai_col}")
    ...
    SEG_OFFSET = 2
    # -----------------------------------------------------------------------
    # 4b. 列インデックスの動的解決 (ユーザー提案の header_map 採用)
    # -----------------------------------------------------------------------
...
def _create_ppm_analysis_sheet_ifrs(workbook, analysis_sheet_name, used_sheet_names, filing_pairs, debug_log,
    ...
    # Fallback: セグメント利益が見つからない場合、「当期損益」「当期利益」「当期損失」「セグメント損益」を探す
    if not profit_label_candidates:
        _profit_fallback_keywords = ["当期損益", "当期利益", "当期損失", "セグメント損益"]
    ...
    OTHER_KW = ["その他", "以外", "Other"]
    ADJ_KW   = ["調整", "消去", "内部", "Adjustment", "Elimination"]
    # 既存の合計列を特定するためのキーワード（「報告セグメント」単体も合計として扱う）
    TOTAL_KW = ["合計", "計", "Total", "全社", "全体", "連結", "単体"]

    def _get_category(col_idx):
        val = str(analysis_ws.cell(1, col_idx).value or "")
        # 「報告セグメント」という名前のみの列は、IFRS等では既存の合計列であることが多いため既存合計扱いとする
        if val == "報告セグメント" or val == "全体":
            return "existing_total"
        if any(kw in val for kw in TOTAL_KW):
            return "existing_total"
    ...
    for _ci in range(3, max_col + 1):
        cat = _get_category(_ci)
        header = analysis_ws.cell(1, _ci).value
        # データ行も保持（大規模な場合は注意が必要だが、セグメント数は通常少ない）
        data = []
        for _ri in range(2, total_rows + 1):
            data.append(analysis_ws.cell(_ri, _ci).value)
        all_col_data.append({"header": header, "cat": cat, "original_idx": _ci, "data": data})
    ...
    for _ri_idx, _row in enumerate(range(2, total_rows + 1)):
        ...
        # ③ 全社
        _xbrl_zen = _get_gepv_val(netsales_elems, ['全体', '連結', 'Consolidated'], _pstr, _gv + _adj_sum)
        _zv = _xbrl_zen if _xbrl_zen is not None else (_gv + _adj_sum)
    ...
    def _norm_lbl(lbl):
        """ラベルの全角スペース・ASCII空白を除去して正規化"""
        return str(lbl).replace('\u3000', '').strip() if lbl else ''

    def _ppm_label_to_acq(ppm_lbl):
        """PPMラベルをデータ取得シートのcanonicalラベルに変換
        IFRS分析シートのラベルには全角スペースが付くため正規化して比較する"""
        if ppm_lbl is None:
            return None
        norm = _norm_lbl(ppm_lbl)
        for acq_lbl in acq_canonical_labels:
            acq_norm = _norm_lbl(acq_lbl)
            if norm == acq_norm:
                return acq_lbl
            if norm.startswith(acq_norm) or acq_norm.startswith(norm):
                return acq_lbl
        return ppm_lbl  # fallback
